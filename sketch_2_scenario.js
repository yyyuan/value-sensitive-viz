...
function setup() {
  ...
  //
  buttonData = createButton('Visualization View');
  buttonData.position(leftX-20, 20);
  buttonData.mousePressed(buttonDataEvent);

  buttonScenario = createButton('Text View');
  buttonScenario.position(buttonData.x+180, 20);
  buttonScenario.mousePressed(buttonScenarioEvent);
  buttonScenario.addClass('active');
  //
  ...
  yRange = {}; yRange.min = 545; yRange.max = 425;
}
...
function draw() {
  ...
  //
    ...
    if (currentType == 1 || currentType == 2){
      var text = "<u>Total Defendants</u>: 3000&nbsp;&nbsp;&nbsp;&nbsp;<u>Prediction Errors</u>: "+ errors+ " ("+errorRate+"%)"+"&nbsp;&nbsp;&nbsp;&nbsp;<u>Disparity</u>: " +data.dfp + " ("+Math.floor(data.dfp/3000*10000/100)+"%)";
    } else {
      var text = "<u>Total Defendants</u>: 3000&nbsp;&nbsp;&nbsp;&nbsp;<u>Prediction Errors</u>: "+ errors + " ("+errorRate+"%)";
    }
    ...
    var data22 = Math.floor(10000-data.accuracy*10000)/100;
    ...
    drawAreaLabelLeft(leftX,topY+dy, mainBlue, "TRUE POSITIVE","Predict defendants will reoffend and the prediction is CORRECT");
    drawAreaLabelLeft(leftX,centerY+dy, mainPink, "FALSE POSITIVE","Predict defendants will reoffend but the prediction is WRONG");
    //
    drawAreaLabelRight(rightX-130,topY+dy, mainPink, "FALSE NEGATIVE","Predict defendants will NOT reoffend but the prediction is WRONG");
    drawAreaLabelRight(rightX-130,centerY+dy, mainBlue, "TRUE NEGATIVE","Predict defendants will NOT reoffend and the prediction is CORRECT");
    //
    // call drawLegend
...
function drawScenario(data){
  ...
  if (currentType == 0){
    stroke(0,0,0,50);
    line(380, 100, canvasW - margin*8 , 100);
    constructScenario(400,120,data);
    line(380, 550, canvasW - margin*8 , 550);
  } else {
    stroke(0,0,0,50);
    line(380, 100, canvasW - margin*8 , 100);
    constructScenario(400,120,data);
    line(380, 650, canvasW - margin*8 , 650);
  }
}
...
function constructScenario(x,y,data){
  ...
  if(currentType == 0){
    ...
    content = content + "<strong>TRUE NEGATIVE: "+ data.tna1 + " White American </strong> and <strong>" + data.tna0 +" African American</strong> defendants who are predicted NOT to reoffend will actually NOT reoffend (Difference "+diffTN+").</li><li>";
    content = content + "<strong>TRUE POSITIVE: "+ data.tpa1 + " White American </strong> and <strong>" + data.tpa0 + " African American</strong> defendants who are predicted to reoffend will actually reoffend (Difference "+diffTP+").</li></ul><br>";

    scenario2content.html(content);
    scenario2content.position(x,y);
    stroke(0,0,0,50);
  } else {
    content = "Overall, among <strong>1600</strong> defendants, <strong>"+errors+" ("+errorRate+"%)</strong> are incorrectly predicted. <br><br>";
    //
    var diffFN = Math.abs(data.fna1 - data.fna0);
    var diffFP = Math.abs(data.fpa1 - data.fpa0);
    content = content + "Among these <strong>"+errors+"</strong> incorrect predictions:<br><ul><li>";
    content = content + "<strong>FALSE NEGATIVE: "+ data.fna1 + " male </strong> and <strong>" +data.fna0+" female</strong> defendants who are predicted NOT to reoffend will actually reoffend (Difference "+diffFN+").</li><li>";
    content = content + "<strong>FALSE POSITIVE: "+ data.fpa1 + " male </strong> and <strong>" +data.fpa0+" female</strong> defendants who are predicted to reoffend will actually NOT reoffend (Difference "+diffFP+").</li></ul><br>";
    //
    var diffTP = Math.abs(data.tpa0 - data.tpa1);
    var diffTN = Math.abs(data.tna0 - data.tna1);
    content = content + "Among these <strong>"+corrections+"</strong> correct predictions:<br><ul><li>";

    content = content + "<strong>TRUE NEGATIVE: "+ data.tna1 + " male </strong> and <strong>" + data.tna0 +" female</strong> defendants who are predicted NOT to reoffend will actually NOT reoffend (Difference "+diffTN+").</li><li>";
    content = content + "<strong>TRUE POSITIVE: "+ data.tpa1 + " male </strong> and <strong>" + data.tpa0 + " female</strong> defendants who are predicted to reoffend will actually reoffend (Difference "+diffTP+").</li></ul><br>";
    //
    scenario2content.html(content);
    scenario2content.position(x,y);
  }
...
function buttonDataEvent(){
  ...
  //view = 0;
  //window.open("explore_2_data.html",'_self');
}
